...
public class CoffeeDaoImpl implements CoffeeDao {
	...
	public ArrayList<String> getAllCoffeeNameDetails()  {
		 Session session=sessionFactory.openSession();		
			Transaction transaction=session.beginTransaction();
			Query<String> query = session.createQuery("select distinct(cf.coffeeName) from Coffee cf"); 
		/*
		 * List<Coffee> coffeeList = query.getResultList(); for(Coffee
		 * coffeeLists:coffeeList) { coffeeNameList.add(coffeeLists.getCoffeeName()); }
		 */
			List<String> coffeeNameList=query.getResultList();
			transaction.commit();
			session.close();
			//factory.close();
			return (ArrayList<String>) coffeeNameList;
	}
